...
describe('registerValidator', () => {
  it('adds a custom validator', () => {
    registerValidator('svelteCustomEven', (value) => {
      if (typeof value === 'number' && value % 2 !== 0) return 'Must be even'
      return null
    })
    expect(validateField(3, [{ rule: 'svelteCustomEven' }])).toHaveLength(1)
    expect(validateField(4, [{ rule: 'svelteCustomEven' }])).toHaveLength(0)
  })
})
