...
6. **Factory functions** — handlers, hooks, and services are always created via factory functions that receive dependencies
7. **Base config in settings/** — shared defaults live in `settings/schema.ts`, `settings/handlers.ts`, `settings/hooks.ts`
