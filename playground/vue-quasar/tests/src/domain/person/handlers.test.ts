...
import { createPersonHandlers, createPersonService, PersonSchema } from "@ybyra/demo";
...
describe("createPersonHandlers", () => {
  ...
  it("update validates, updates, toasts success, and navigates", () => {
    const { state, component, form } = createMockContext(PersonSchema, vi.fn).scopes(scopes).values({ id: "1", name: "Bob" });
  ...
  it("destroy confirms dialog, destroys, and navigates when not on index", async () => {
    const { state, component } = createMockContext(PersonSchema, vi.fn).scopes(scopes).scope(Scope.edit).values({ id: "99" });
  ...
  it("destroy calls table's reload when the scope is 'index'", async () => {
    const { state, component, table } = createMockContext(PersonSchema, vi.fn).scopes(scopes).scope(Scope.index).values({ id: "1" });
