...
Anhanga's React Native integration ships with ready-to-use components via `@anhanga/react-native`. Forms, tables, field renderers, and action buttons all work out of the box — **no customization required** to get a fully functional CRUD.

## When to Customize

You might want to customize when:

- **Branding** — your app has specific input styles, spacing, or color tokens
- **Specialized inputs** — a camera picker, barcode scanner, or map widget that isn't built in
- **Different component library** — you prefer React Native Paper, NativeBase, or Tamagui
